...
from app import process_domain, create_csv, create_xlsx, CONFIG, task_lock, task_storage, generate_task_id
...
def main():
    ...
    blocked = sum(1 for r in results if r['resolved'] == False)
    
    print(f"总计: {len(results)} | 成功: {success} | 失败: {failed} | 停止解析: {blocked}")
    print(f"耗时: {elapsed:.1f}秒")
    print(f"平均: {elapsed/len(results):.2f}秒/域名")
    print()
    
    # 导出CSV
    output_file = f"domain_report_{int(time.time())}.csv"
